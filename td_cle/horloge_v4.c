...
int main()
{
	...
	while(1)
	{
		...
		if(chrono_on == (1))
		{
			temps = /* (SDL_GetDelay/1000) */ - depart_chrono;
		}
...
//		printf(" %d",event.button.y);
		...
		{
			mode = mode * (-1);
			depart_chrono = SDL_GetTicks() / 1000;
		}
		if((event.button.x>(25))&&(event.button.x<25+180)&&(event.button.y>50)&&(event.button.y<50+80))
		{
			chrono_on = chrono_on * (-1);
//			depart_chrono = SDL_GetDelay;
		}
	}
