...
// SOMMAIRE
//
// 0. AFFICHAGE DE TEXTE AVEC SDL_ttf ... ligne   31
// 1. VARIABLES, CONSTANTES ............. ligne   49
// 2. AFFICHAGE ......................... ligne   72
// 3. GESTION D'ÉVÉNEMENTS .............. ligne  170
// 4. DESSIN D'OBJETS ................... ligne  395
// 5. ÉCRITURE DE TEXTE ................. ligne  783
// 6. LECTURE D'ENTIER .................. ligne  930
// 7. GESTION DU TEMPS .................. ligne  948
// 8. VALEUR ALÉATOIRES ................. ligne 1008
// 9. DIVERS ............................ ligne 1028
// 10. AJOUTS PERSONNELS ................ ligne 1047
// ################################################
...
//	SDL_WM_SetCaption("UVSQ -- Machines Virtuelles",NULL); /*titre nul*/
	SDL_WM_SetCaption("Ma Yolo fenetre",NULL); /*titre genial*/
...
// A ne mettre que si on est en local, sur les ordi des étudiants, c'est trop lent
			...
			#endif
#endif
			printf("\rEn attente de clic ... %4d %4d           \r",event.motion.x,HEIGHT - event.motion.y);
			fflush(stdout);
			}
		/* Si l'utilisateur a demandé à fermer la fenêtre, on quitte */
		if (event.type == SDL_QUIT) exit(0);
...
// A ne mettre que si on est en local, sur les ordi des étudiants, c'est trop lent
	...
	#endif
#endif
	while (SDL_PollEvent(&event) && encore)
		{
		/* Si l'utilisateur clique avec la souris */
		if ((event.type == SDL_MOUSEBUTTONDOWN) && (event.button.button == SDL_BUTTON_LEFT))
			{
...
void affiche_image_perso(char *nomfic, POINT bg, int w, int h,int rg,int vr, int bl)
{
SDL_Surface *image = NULL;
SDL_Rect rect;
rect.x = bg.x; rect.y = HEIGHT-bg.y;
...
if (image == NULL) fprintf(stderr,"Impossible charger l'image : %s\n",nomfic);
/* On blitte par-dessus l'écran */
SDL_SetColorKey(image,SDL_SRCCOLORKEY,SDL_MapRGB(image->format,rg,vr,bl));
SDL_BlitSurface(image, NULL, SDL_screen, &rect);
if (SDL_AFFICHE_AUTO) affiche_all();
}
...
int largeur_texte(char *texte, int taille)
	...
	POINT p;
	p.x = -10; p.y = -10;
	aff_pol(NULL,taille,p,black); // Appel de aff_pol pour fixer la valeur de ___pol
...
void pol_style(int style)
	...
	POINT p;
	p.x = -10; p.y = -10;
	aff_pol(NULL,10,p,black); // Appel de aff_pol pour fixer la valeur de ___pol
...
// #####################
// 10. AJOUTS PERSONNELS
// #####################

	// 10.1 dessine des lignes verticales

void draw_line_V(int ecartV,COULEUR coul)
{
	POINT P1,P2;
	P1.x=ecartV;P1.y=0;
	P2.x=ecartV;P2.y=5000;
	...
	// 10.2 dessine des lignes horizontale

void draw_line_H(int ecartH,COULEUR coul)
{
	POINT P1,P2;
	P1.y=ecartH;P1.x=0;
	P2.y=ecartH;P2.x=5000;
	...
	// 10.3 dessine un quadrillage

void draw_quadrillage(int ecartV,int ecartH,COULEUR coul)
{
	draw_line_V(ecartV,coul);
	draw_line_H(ecartH,coul);
}

	// 10.4 dessine un hexagone


#include <math.h>

void draw_hexagone(POINT p,int taille,COULEUR coul)
{
	float a=sqrt((taille*taille)-(taille/2)*(taille/2));
	
	POINT p1,p2,p3,p4,p5,p6;
	p1.x=p.x-taille;p1.y=p.y;
	p2.x=p.x-(taille/2);p2.y=p.y+a;
	p3.x=p.x+(taille/2);p3.y=p.y+a;
	p4.x=p.x+taille;p4.y=p.y;
	p5.x=p.x+(taille/2);p5.y=p.y-a;
	p6.x=p.x-(taille/2);p6.y=p.y-a;
	
	draw_line(p1,p2,coul);
	draw_line(p2,p3,coul);
	draw_line(p3,p4,coul);
	draw_line(p4,p5,coul);
	draw_line(p5,p6,coul);
	draw_line(p6,p1,coul);
}

	// 10.5 dessine un hexagone plein

void draw_fill_hexagone(POINT p,int taille,COULEUR coul)
{
	float a=sqrt((taille*taille)-(taille/2)*(taille/2));
	...
	//draw_pixel(p3,blanc);
	//draw_pixel(p4,vert);
	//draw_pixel(p5,marron);
	//draw_pixel(p6,jaune);
	
	draw_fill_triangle(p1,p2,p,coul);
	//draw_fill_triangle(p2,p3,p,coul);
	draw_fill_triangle(p3,p4,p,coul);
	draw_fill_triangle(p4,p5,p,coul);
	//draw_fill_triangle(p5,p6,p,coul);
	draw_fill_triangle(p6,p1,p,coul);
	draw_fill_rectangle(p2,p5,coul);
	
}

	// 10.6 dessine un polygone

void draw_polygone(int nbre_cote,int rayon_cercle,POINT p,COULEUR coul)
{
	POINT tableau[nbre_cote];
	tableau[0].x=p.x+rayon_cercle;tableau[0].y=p.y;
	
	int i;
	
	for(i=1;i<nbre_cote;i++)
	{
		tableau[i].x=p.x+(cos(((2*M_PI)/nbre_cote)*i)*rayon_cercle);
		tableau[i].y=p.y+(sin(((2*M_PI)/nbre_cote)*i)*rayon_cercle);
	}
	
	for(i=0;i<(nbre_cote-1);i++)
	{
		draw_line(tableau[i],tableau[i+1],coul);
	}
	
	draw_line(tableau[nbre_cote-1],tableau[0],coul);
}

	// 10.6 dessine un polygone plein

void draw_fill_polygone(int nbre_cote,int rayon_cercle,POINT p,COULEUR coul)
{
	POINT tableau[nbre_cote];
	tableau[0].x=p.x+rayon_cercle;tableau[0].y=p.y;
	...
	for(i=0;i<(nbre_cote-1);i++)
	{
		draw_fill_triangle(tableau[i],tableau[i+1],p,coul);
	}
	
	draw_fill_triangle(tableau[nbre_cote-1],tableau[0],p,coul);
}
