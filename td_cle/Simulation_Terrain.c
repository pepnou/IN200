...
#define probabilite_depart_feu 1
#define probabilite_fin_feu 50
#define probabilite_pousse_arbre 15
#define probabilite_cendre_froide 25
	...
	for(i=1;i<largeur+1;i++)
		...
		for(j=1;j<hauteur+1;j++)
		{
			...
			switch(tab[i][j])
			{
				...
				case 4 : draw_fill_rectangle(p1,p2,couleur_RGB(0,206,0)); break ;
		...
		for(j=1;j<hauteur+1;j++)
		{
			...
			switch(tab[i][j])
			{
				...
				else
				{
					...
					{
						a=alea_int(101);
						if(a>(100-probabilite_depart_feu))
						{
							tab[i][j] = 8;
						}
					}
				...
				if((regarder_autour(tab,i,j,3)>0)||(regarder_autour(tab,i,j,4)>0))
				{
					...
					{
						if(a-5>(100-probabilite_propagation_plante))
						{
							tab[i][j] = 4;
						}
					}
				...
				{
					if((a-45>(100-probabilite_depart_plante))&&(regarder_autour(tab,i,j,7) == 0)&&(regarder_autour(tab,i,j,8) == 0))
						{
							tab[i][j] = 4;
						}
				}
